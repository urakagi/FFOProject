...
public class CounterRowView extends LinearLayout implements OnTouchListener {
	...
	public class UView extends View {
		...
		protected void onDraw(Canvas canvas) {
			...
			paint.setTextSize(mSize * 0.6f);
			canvas.drawText(mSubText, 0, mSize * 0.6f, paint);
		}
	}
	...
	public boolean onTouch(View v, MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (Math.abs(deltaY) > this.getHeight() / 2) {
				...
				setValue(mBaseValue + delta, delta);
				return true;
			}

			int newValue;
			float deltaX = event.getX() - mBaseX;
			int delta = (int) (deltaX / DPP);
			if (mMirror) {
				newValue = mBaseValue - delta;
			} else {
				newValue = mBaseValue + delta;
			}
			if (value != newValue) {
				setValue(newValue, mMirror ? -delta : delta);
			}
			return true;
		}
	...
	private void setValue(int value, int delta) {
		this.value = value;
		PreferenceManager.getDefaultSharedPreferences(getContext()).edit()
				.putInt(mId, value).commit();
		_amount.mText = String.valueOf(value);
		_amount.mSubText = "(" + (delta < 0 ? String.valueOf(delta) : "+" + String.valueOf(delta)) + ")";
		_amount.invalidate();
	}
